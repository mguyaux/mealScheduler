...
type RestOf<T extends ISchedule | NewSchedule> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class ScheduleService {
  ...
  protected convertDateFromClient<T extends ISchedule | NewSchedule | PartialUpdateSchedule>(schedule: T): RestOf<T> {
    return {
      ...schedule,
      date: schedule.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSchedule: RestSchedule): ISchedule {
    return {
      ...restSchedule,
      date: restSchedule.date ? dayjs(restSchedule.date) : undefined,
    };
  }
